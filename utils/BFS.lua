...
BFS.find = function(start, goal, is_walkable, map, size)
  local q = Queue()
  local visited = {}
...
BFS.is_inside = function(node, size) return node.x >= 0 and node.x <= size and node.y >= 0 and node.y <= size end
...
BFS.backtrack = function(current, result)
  ...
  return BFS.backtrack(current.parent, result)
end


BFS.print_map = function(map, visited, goal, size)
