...
function d:part1(input_data)
  ...
  for _, value in ipairs(input_data) do
    ...
    table.insert(cc, { hand = hand, bid = bid })
  end
  table.sort(cc, function(a, b)
    return d.play(a.hand, b.hand) or false
  end)
  local result = 0
...
function d.play(a, b, t)
  ...
  if t.joker then
    a_type = d.count_jokers(a, a_type)
    b_type = d.count_jokers(b, b_type)
  end
  if a_type == b_type then return d.get_higher_value_hand(a, b, t) end
  return a_type < b_type
end

function d.count_jokers(hand, type)
  local jokers = Counter.count_char("J", hand)
  if jokers > 0 then type = d.raise_type(type, jokers) end
  return type
end
